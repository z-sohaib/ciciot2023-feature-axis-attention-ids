from __future__ import annotations

import argparse
import json
import sys
from pathlib import Path
from typing import Any

import numpy as np
import torch

ROOT = Path(__file__).resolve().parents[1]
SRC = ROOT / "src"
if str(SRC) not in sys.path:
    sys.path.insert(0, str(SRC))

from ciciot2023_ids.config import load_config
from ciciot2023_ids.metrics import classification_metrics
from ciciot2023_ids.models.factory import build_model
from ciciot2023_ids.train import load_processed_dataset, make_loader, predict


def main() -> int:
    parser = argparse.ArgumentParser(description="Evaluate CICIoT2023 test-time perturbation robustness.")
    parser.add_argument("--config", required=True, help="Config used to train the selected model.")
    parser.add_argument("--model-path", required=True, help="Path to model.pt.")
    parser.add_argument("--processed-path", default=None, help="Optional processed NPZ override.")
    parser.add_argument("--output", required=True, help="Output JSON path.")
    parser.add_argument("--noise-std", nargs="*", type=float, default=[0.01, 0.03, 0.05])
    parser.add_argument("--feature-dropout", nargs="*", type=float, default=[0.05, 0.10, 0.20])
    parser.add_argument("--seed", type=int, default=42)
    args = parser.parse_args()

    config = load_config(args.config)
    processed_path = Path(args.processed_path or config["data"]["processed_path"])
    print(f"[objective3] Loading CICIoT2023 processed data from {processed_path}", flush=True)
    x_train, x_val, x_test, y_train, y_val, y_test, class_names = load_processed_dataset(processed_path)
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"[objective3] Loaded test samples={len(y_test)} features={x_test.shape[1]} device={device}", flush=True)

    model = build_model(config, input_features=x_train.shape[1], num_classes=len(class_names)).to(device)
    model.load_state_dict(torch.load(args.model_path, map_location=device))
    model.eval()

    batch_size = int(config["training"]["batch_size"])
    print("[objective3] Evaluating clean test set", flush=True)
    clean_metrics = _evaluate(model, x_test, y_test, class_names, batch_size, device)
    results: dict[str, Any] = {
        "config": args.config,
        "model_path": args.model_path,
        "processed_path": str(processed_path),
        "seed": args.seed,
        "clean": clean_metrics,
        "perturbations": [],
    }

    rng = np.random.default_rng(args.seed)
    for std in args.noise_std:
        print(f"[objective3] Evaluating Gaussian noise std={std}", flush=True)
        perturbed = x_test + rng.normal(0.0, std, size=x_test.shape).astype(np.float32)
        metrics = _evaluate(model, perturbed.astype(np.float32), y_test, class_names, batch_size, device)
        results["perturbations"].append(_payload("gaussian_noise", std, clean_metrics, metrics))

    for rate in args.feature_dropout:
        print(f"[objective3] Evaluating feature dropout rate={rate}", flush=True)
        mask = rng.random(x_test.shape) >= rate
        perturbed = x_test * mask.astype(np.float32)
        metrics = _evaluate(model, perturbed.astype(np.float32), y_test, class_names, batch_size, device)
        results["perturbations"].append(_payload("feature_dropout", rate, clean_metrics, metrics))

    output_path = Path(args.output)
    output_path.parent.mkdir(parents=True, exist_ok=True)
    output_path.write_text(json.dumps(results, indent=2), encoding="utf-8")
    print(f"Saved robustness evaluation to {output_path}")
    return 0


def _evaluate(
    model: torch.nn.Module,
    x: np.ndarray,
    y: np.ndarray,
    class_names: list[str],
    batch_size: int,
    device: torch.device,
) -> dict[str, Any]:
    loader = make_loader(x.astype(np.float32), y.astype(np.int64), batch_size, shuffle=False)
    predictions = predict(model, loader, device)
    return classification_metrics(y, predictions, class_names)


def _payload(kind: str, value: float, clean: dict[str, Any], metrics: dict[str, Any]) -> dict[str, Any]:
    return {
        "type": kind,
        "value": float(value),
        "accuracy": metrics["accuracy"],
        "macro_f1": metrics["macro_f1"],
        "weighted_f1": metrics["weighted_f1"],
        "accuracy_drop": float(clean["accuracy"] - metrics["accuracy"]),
        "macro_f1_drop": float(clean["macro_f1"] - metrics["macro_f1"]),
        "weighted_f1_drop": float(clean["weighted_f1"] - metrics["weighted_f1"]),
        "per_class_false_positive_rate": metrics["per_class_false_positive_rate"],
        "classification_report": metrics["classification_report"],
    }


if __name__ == "__main__":
    raise SystemExit(main())
